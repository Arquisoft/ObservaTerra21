...
public class Admin extends Controller {
    ...
    public static Result newOrganization() {
        Form<Organization> form = organizationForm.bindFromRequest();
    	  if(form.hasErrors()) {
    	    return badRequest(
    	      views.html.organization.render(Organization.all(),organizationForm)
    	    );
    	  } else {
    		  Organization ind = form.get();
    		  Organization.create(ind);
    	    return redirect(routes.Application.showIndicators());  
    	  }    
      }
      
      public static Result deleteOrganization(String code) {
